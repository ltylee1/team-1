...
class DatabaseReader(models.Model):
	...
	def readData(self):
		...
		query = "SELECT * FROM uw_dashboard_program AS p, uw_dashboard_target_population AS t, uw_dashboard_geo_focus_area AS gfa, uw_dashboard_donor_engagement AS de WHERE"

		query += " p.program_andar_number = t.program_andar_number_id AND p.program_andar_number = gfa.program_andar_number_id AND p.program_andar_number = de.program_andar_number_id AND"
