...
class DatabaseReader(models.Model):
	...
	def readData(self):
		...
		query = "SELECT p.*, GROUP_CONCAT(DISTINCT pe.element_name SEPARATOR ', ') as element_names, GROUP_CONCAT(DISTINCT pe.specific_element SEPARATOR ','), GROUP_CONCAT(DISTINCT de.donor_engagement SEPARATOR ','), GROUP_CONCAT(DISTINCT t.target_population SEPARATOR ',') as target_population, GROUP_CONCAT(DISTINCT l.postal_code SEPARATOR ',') as postal_codes, COUNT(DISTINCT l.postal_code) as postal_count, SUM(DISTINCT p.allocation), MIN(DISTINCT p.grant_start_date), MAX(DISTINCT p.grant_end_date), GROUP_CONCAT(DISTINCT gfa.city SEPARATOR ',') as city, GROUP_CONCAT(DISTINCT gfa.city_grouping SEPARATOR ',') as city_grouping, GROUP_CONCAT(DISTINCT a.agency_name SEPARATOR ',') as agency_name"
